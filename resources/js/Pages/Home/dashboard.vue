    ...
    <v-container fluid>
      <template v-if="Truthty(!temporada)">
        <v-card color="background" class="shadow-md px-4 py-2">
          <v-card-title class="text-center" style="font-size: 30px">No existe ninguna temporada activa en este momento
            !</v-card-title>
            ...
            <v-card name="totalUsuarios">
              <v-card-item>
                ...
                <v-card-text class="py-0">
                  <template v-for="(item, index) in data.totalUsuarios.generos" :key="'genero-' + index">
            ...
            <v-card name="registrosNuevos">
              <v-card-item>
                ...
                <v-card-title class="d-flex justify-space-between">
                  ...
                  <v-chip :prepend-icon="chips.usuariosNuevos.icon" :color="chips.usuariosNuevos.color" variant="tonal"
                    class="text-caption">
                    {{ chips.usuariosNuevos.value }}</v-chip>
                </v-card-title>
                <v-card-text class="">
                  <template v-for="(item, index) in data.usuariosNuevos.generos" :key="'genero-' + index">
                    <div class="d-flex justify-space-between">
                      <text>{{ item.nombre }} :</text>
                      <text>{{ item.total }}</text>
                    </div>
            ...
            <v-card name="inscritos">
              <v-card-item>
                ...
                <v-card-title class="d-flex justify-space-between">
                  ...
                  <v-chip :prepend-icon="chips.usuariosInscritos.icon" :color="chips.usuariosInscritos.color"
                    variant="tonal" class="text-caption">
                    {{ chips.usuariosInscritos.value }}</v-chip>
                </v-card-title>
                <v-card-text class="py-0">
                  <template v-for="(item, index) in data.usuariosInscritos.generos" :key="'genero-' + index">
                    <div class="d-flex justify-space-between">
                      <text>{{ item.nombre }} :</text>
                      <text>{{ item.total }}</text>
                    </div>
                ...
                <v-card-text class="pt-0">
                  <template v-for="(item, index) in data.usuariosInscritos.roles" :key="'rol-' + index">
                    <div class="d-flex justify-space-between">
                      <text>{{ item.nombre }} :</text>
                      <text>{{ item.total }}</text>
                    </div>
            ...
            <v-card>
              <v-card-item>
                ...
                <v-card-title class="d-flex justify-space-between">
                  ...
                  <v-chip :prepend-icon="chips.cursos.icon" :color="chips.cursos.color" variant="tonal"
                    class="text-caption">
            ...
            <v-card>
              <v-card-item>
                ...
                <v-card-title class="d-flex justify-space-between">
                  ...
                  <v-chip :prepend-icon="chips.inscripciones.icon" :color="chips.inscripciones.color" variant="tonal"
                    class="text-caption">
        ...
        <v-row>
          <v-col cols="12" lg="4" md="6" sm="6">
            <v-card>
              ...
              <v-list max-height="25rem">
                <v-list-item v-for="(inscripcion, index) in data.inscripcionesCurriculum" :key="index"
                  class="min-h-auto">
                  <v-list-item-content class="d-flex">
                    <v-list-item-title class="white-space-normal w-70">{{
                      inscripcion.curriculum_nombre
                    }}</v-list-item-title>
                    <v-list-item-subtitle class="overflow-visible d-flex align-center justify-end w-15">{{
                      inscripcion.cantidad
                    }}</v-list-item-subtitle>
                    <v-list-item-subtitle class="overflow-visible d-flex align-center justify-end w-15">{{
                      getBadgePercentage(inscripcion.percentage).value
                    }}</v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </v-list>
          ...
          <v-col cols="12" lg="4" md="6" sm="6">
            <v-card>
              ...
              <v-list max-height="25rem">
                <v-list-item v-for="(inscripcion, index) in data.inscripcionesCiclo" :key="index" class="min-h-auto">
                  <v-list-item-content class="d-flex ">
                    <v-list-item-title class="white-space-normal w-70">{{ inscripcion.curriculum_nombre }} -
          ...
          <v-col cols="12" lg="4" md="6" sm="6">
            <v-card>
              ...
              <v-list max-height="25rem">
                <v-list-item v-for="(inscripcion, index) in data.inscripcionesPais" :key="index" class="min-h-auto">
                  <v-list-item-content class="d-flex ">
                    <v-list-item-title class="white-space-normal w-70">{{
          ...
          <v-col cols="12" lg="4" md="6" sm="6">
            <v-card>
              ...
              <v-list max-height="25rem">
                <v-list-item v-for="(inscripcion, index) in data.inscripcionesRegion" :key="index" class="min-h-auto">
                  <v-list-item-content class="d-flex ">
                    <v-list-item-title class="white-space-normal w-70">{{ inscripcion.pais_nombre }} -
