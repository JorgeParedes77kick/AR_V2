...
const props = defineProps({
  ...
  inscripciones: { type: Object, default: [] },
});
  ...
  <MainLayout>
    <v-container fluid>
      ...
      <v-card color="background" class="shadow-md px-4 py-2">
        <v-card-title class="text-center ">ALUMNOS {{ grupopequeno.ciclo.curriculum.nombre }} {{
          grupopequeno.ciclo.nombre }}
          - {{ grupopequeno.temporada.prefijo }} </v-card-title>
        <v-divider></v-divider>
        <v-row>
          ...
          <v-col cols="12">
            <v-data-table :headers="headers" :items="inscripciones" :items-per-page="20" class="elevation-1 rounded">
      ...
      <v-card :title="`#${userSelect?.id} - ${userSelect?.nombreCompleto}`">
        ...
        <v-card-text>
          ...
          <v-row>
            ...
            <v-col cols="12" sm="6" class="py-1">{{ userSelect?.persona?.region?.pais?.nombre }}</v-col>
          </v-row>
          <v-row>
            <v-col cols="12" sm="6" class="py-1"><strong>Nacionalidad: </strong></v-col>
            <v-col cols="12" sm="6" class="py-1">{{ userSelect?.persona?.nacionalidad?.nombre }}</v-col>
          </v-row>
          <v-row>
            <v-col cols="12" sm="6" class="py-1"><strong>Estado Civil: </strong></v-col>
            <v-col cols="12" sm="6" class="py-1">{{ userSelect?.persona?.estado_civil?.estado }}</v-col>
          </v-row>
          <v-row>
