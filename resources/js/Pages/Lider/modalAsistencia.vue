...
const buscarAsistencias = async () => {
  loading.value = true;
  try {
    const response = await axios.get(route('mis-salones.asistencia', props.idGrupo));
    const { data } = response;
...
const onClickSave = async () => {
  loading.value = true;
  const body = { inscrito: [], presente: [], ausente: [], recuperado: [], noAplica: [], otros: [] };
...
<template>
  ...
  <v-dialog width="auto">
    <template v-slot:activator="{ props: activatorProps }">
      <v-btn color="success" @click="buscarAsistencias" v-bind="activatorProps"> Asistencia </v-btn>
    </template>
