  ...
  if (inscripcion && estado_inscripcion_id == inscripcion.estado_inscripcion_id) {
    ...
    const html = inscripcion
      ...
      : `Estas seguro de inscribir a <b>${usuario.value.fullNombre}</b>
          A la temporada ${temporada.prefijo}, grupo pequeño <b>${curriculum.nombre}</b> en el <b>${ciclo.nombre}</b>
    ...
    <v-container>
      <v-card color="background" class="px-4 py-2">
        ...
        <v-form @submit="buscarPorEmail" ref="formEmail" lazy-validation>
          <v-row>
            <v-col>
              <v-text-field id="email" name="email" label="CORREO ELETRÓNICO" v-model="inputEmail.email"
                placeholder="introduzca el correo electrónico" :rules="validate('Email', 'required|email')"
                :error-messages="inputEmail.errors.email" clearable />
            </v-col>
        ...
        <v-data-table v-if="inscripciones.length > 0" :headers="headers" :items="inscripciones" :items-per-page="20"
        ...
        <FormInscripcion :temporadas="temporadas" :curriculums="curriculums" :dias="dias" :estados="estados"
          :usuario="usuario" />
      </v-card>
    </v-container>
  </MainLayout>
