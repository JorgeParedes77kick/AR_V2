...
const submit = async () => {
  ...
  try {
    ...
    const response = await axios.put(route('globals.update', globalSelect.value.id), globalSelect.value);
  ...
  <MainLayout>
    <v-container>
      <v-card color="background" class="px-4 py-2" title="Globales" :loading="loading">
        <v-row>
          <v-col>
            <v-data-table :loading="loading" :headers="headers" :items="globales" :items-per-page="10"
    ...
    <v-dialog v-model="openShow" width="auto" min-width="25rem" persistent>
      <v-card :title="`#${globalSelect?.id} - ${globalSelect?.nombre}`" :loading="loading">
        <v-card-text>
          <v-row>
            <v-col cols="12" sm="6" class="py-1 d-flex justify-start justify-sm-center align-center"><v-label
                class="text-input">Valor: </v-label></v-col>
            <v-col cols="12" sm="6" class="py-1">
              <v-text-field id="valor" name="valor" v-model="globalSelect.valor" :rules="validate('valor', 'required')"
                color="input" /></v-col>
          </v-row>
          <v-row>
            <v-col cols="12" sm="6" class="py-1 d-flex justify-start justify-sm-center align-center"><v-label
                class="text-input">Tipo: </v-label></v-col>
            <v-col cols="12" sm="6" class="py-1">
              <v-select id="tipo" name="tipo" v-model="globalSelect.tipo" :rules="validate('tipo', 'required')"
                :items="tipos_cast" color="input" /></v-col>
          </v-row>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
