    ...
    <MainLayout>
        <v-container fluid>
            <v-card color="background" class="shadow-md px-4 py-2">
                ...
                <v-card-title>
                    <ButtonBack :href="route(miPerfil ? 'home' : 'personas.index')" /> {{ miPerfil ? 'Mi Perfil' :
                        `Nick:
                    ${props.usuario.nick_name}` }}
                </v-card-title>
                <v-card-subtitle>
                    <v-switch id="isDisabled" name="isDisabled" v-model="Editar"
                        :label="Editar ? 'Cancelar Edición' : 'Editar Perfil'" color="primary" />
                </v-card-subtitle>
                <v-card-text>
                    ...
                    <v-form @submit="validateForm" ref="form" lazy-validation>
                        <v-row class="justify-center">
                            <v-col cols="12" sm="8" md="6">
                                <v-img v-if="inputForm.imagenFile" :src="previewImage(inputForm.imagenFile)"
                                    max-width="150" max-height="150" class="d-block mx-auto" />
                                <v-img v-if="typeof inputForm.imagen == 'string' && inputForm.imagen.length > 0"
                                    :src="`/storage/img/perfil/${inputForm.imagen}`" max-width="150" max-height="150"
                        ...
                        <v-row class="justify-center">
                            <v-col cols="12" sm="8" md="6">
                                <v-file-input id="imagen" name="imagen" label="Imagen" v-model="inputForm.imagenFile"
                                    ...
                                    prepend-inner-icon="mdi-camera" @update:modelValue="onChangeFile" :rules="[
                                        (value) =>
                                            value ||
                                            !value.length ||
                        ...
                        <v-row class="row-gap-2">
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="Numero de Documento" name="Numero de Documento"
                                    label="Numero de Documento" v-model="inputForm.dni" color="input" disabled
                                    :rules="validate('dni', 'required')" :error-messages="inputForm.errors.dni"
                                    prepend-inner-icon="mdi-lock-outline" /></v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="email" name="email" label="email" v-model="inputForm.email"
                                    color="input" disabled :rules="validate('email', 'required')"
                                    :error-messages="inputForm.errors.email" prepend-inner-icon="mdi-lock-outline" />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="telefono" name="telefono" label="Telefono"
                                    v-model="inputForm.telefono" color="input" :disabled="isDisabled"
                        ...
                        <v-row>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="nombre" name="nombre" label="Nombre" v-model="inputForm.nombre"
                                    color="input" :disabled="isDisabled" :rules="validate('Nombre', 'required')"
                                    :error-messages="inputForm.errors.nombre" clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="apellido" name="apellido" label="Apellido"
                                    v-model="inputForm.apellido" color="input" :disabled="isDisabled"
                                    :rules="validate('Apellido', 'required')"
                                    :error-messages="inputForm.errors.apellido" clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="fecha_nacimiento" name="fecha_nacimiento" label="Fecha de nacimiento"
                                    v-model="inputForm.fecha_nacimiento" color="input" type="date"
                                    :disabled="isDisabled" :rules="validate('Fecha de nacimiento', 'required')"
                                    :error-messages="inputForm.errors.fecha_nacimiento" clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-select id="genero_id" name="genero_id" label="Genero" v-model="inputForm.genero_id"
                                    color="input" :disabled="isDisabled" :rules="validate('genero', 'required')"
                                    :error-messages="inputForm.errors.genero_id" :items="generos" item-title="nombre"
                                    item-value="id" />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-select id="estado_civil_id" name="estado_civil_id" label="Estado Civil"
                                    ...
                                    :error-messages="inputForm.errors.estado_civil_id" :items="estadosCivil"
                                    item-title="estado" item-value="id" />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="ocupacion" name="ocupacion" label="Ocupación"
                                    v-model="inputForm.ocupacion" color="input" :disabled="isDisabled"
                        ...
                        <v-row>
                            <v-col cols="12" md="4" sm="6">
                                <v-select id="nacionalidad_id" name="nacionalidad_id" label="Nacionalidad"
                                    ...
                                    :error-messages="inputForm.errors.nacionalidad_id" :items="nacionalidades"
                                    item-title="nombre" item-value="id" clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-select id="pais" name="pais" label="País donde te encuentras"
                                    v-model="inputForm.pais" color="input" :disabled="isDisabled"
                                    :error-messages="inputForm.errors.pais" :items="paises" item-title="nombre"
                            ...
                            <v-col cols="12" md="4" sm="6">
                                <v-select id="region_id" name="region_id" label="Región donde te encuentras"
                                    ...
                                    :error-messages="inputForm.errors.region" :items="regiones" item-title="nombre"
                                    item-value="id" clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="ciudad" name="ciudad" label="Ciudad/Comuna" v-model="inputForm.ciudad"
                                    color="input" :disabled="isDisabled" :error-messages="inputForm.errors.ciudad"
                                    clearable />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="direccion" name="direccion" label="Dirección"
                                    v-model="inputForm.direccion" color="input" :disabled="isDisabled"
                                    :error-messages="inputForm.errors.direccion" clearable />
                            </v-col>
                        </v-row>
                        <v-row no-gutters>
                            <v-col>
                        ...
                        <v-row>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="newContrasena" name="newContrasena" label="Nueva Contraseña"
                                    placeholder="Nueva Contraseña" v-model="inputForm.newContrasena" color="input"
                                    type="password" :disabled="isDisabled"
                                    :error-messages="inputForm.errors.newContrasena" clearable autocomplete="off" />
                            </v-col>
                            <v-col cols="12" md="4" sm="6">
                                <v-text-field id="repitaContrasena" name="repitaContrasena" label="Repita Contraseña"
                                    placeholder="Repita Nueva Contraseña" v-model="inputForm.repitaContrasena"
                                    color="input" type="password" :disabled="isDisabled"
                                    :error-messages="inputForm.errors.repitaContrasena" clearable autocomplete="off" />
