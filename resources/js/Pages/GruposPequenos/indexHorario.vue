  ...
  <MainLayout>
    <v-container>
      <v-card color="background" class="px-4 py-2">
        <v-card-title> GRUPOS PEQUEÑOS - HORARIOS </v-card-title>
        <v-card-body>
          <v-row class="pb-2">
            <v-col class="d-flex justify-end">
              <Link :href="route('horario.create')">
              <v-btn color="success" class="ms-auto">
                Nuevo Horario
              </v-btn>
              </Link>
            </v-col>
              ...
              <v-data-table :headers="headers" :items="filteredItems" :items-per-page="20" class="elevation-1 rounded">
                ...
                <template v-slot:[`item.acciones`]="{ item }">
                  <div class="d-flex inline-flex ga-2">
                    ...
                    <Link v-if="item.temporada.activo" :href="route('horario.edit', item)">
